...
namespace sfacd
{
    ...
    void face3::slice(const plane3& p_plane, face3* p_above_faces, int& p_above_count, face3* p_below_faces,
                      int& p_below_count) const
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            const float vertex_dist = p_plane.distance_to(vertex[i]);
            ...
            const int i2 = (i + 1) % 3;
            const float next_dist = p_plane.distance_to(vertex[i2]);
        ...
        if (above_count >= 3)
        {
            ...
            p_above_faces[p_above_count++] = face3{above[0], above[1], above[2]};

            // Construct the above face (quadrilateral)
            if (above_count == 4)
            {
                p_above_faces[p_above_count++] = face3{above[2], above[3], above[0]};
            }
        }
        ...
        {
            // Construct the above face
            p_below_faces[p_below_count++] = face3{below[0], below[1], below[2]};

            // Construct the above face (quadrilateral)
            if (below_count == 4)
            {
                p_below_faces[p_below_count++] = face3{below[2], below[3], below[0]};
            }
        }
    }
}
